...
/* main function */
void vega(double *data, int *markers_start, int *markers_end, int *start, int *end, int *size, double *mean, int *label, int *n, double *be, int *m, double *std, int *n_reg);
...
double update_priority(breakpoint *brks, int i);

/* this function deletes the regions having a size lower than the specified min_region_size */
void delete_outlier(breakpoint *brks, int num_of_brks);
...
void vega(double *data, int *markers_start, int *markers_end, int *start, int *end, int *size, double *mean, int *label, int *n, double *be, int *m, double *std, int *n_reg){
	...
	while( (b.heap_size>0) && (lambda_gradient<=stop_lambda_gradient)){
		...
		while( (b.heap_size>0) && (heap_max(&b)->p > lambda) ){
			...
			if( (max.p > brks[max.id].p) && (brks[max.id].p < lambda) ){			
				...
				tmp_lambda = update_priority(brks, brk_del.prev);
				...
				tmp_lambda = update_priority(brks, brk_del.next);
	...
	delete_outlier(brks, b.heap_size);
	...
	while( (s.heap_size>1) && (heap_max(&s)->p > min_region_size) ){
		...
		if(curr.prev!=-2){
			...
			if(curr.id==0){
				...
				tmp_lambda = update_priority(brks, curr.id);
				curr.p = tmp_lambda;
				if(next.next!=num_of_probes){
					tmp_lambda = update_priority(brks, next.next);
				...
				if( s.heap_size==2 ){
					...
					if(curr.next==num_of_probes){
						...
						tmp_lambda = update_priority(brks, prev.id);
						...
						if( curr.p >= next.p ){
							...
							tmp_lambda = update_priority(brks, prev.id);
							brks[prev.id].p = tmp_lambda;
							tmp_lambda = update_priority(brks, next.id);
